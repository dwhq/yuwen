        ...
        <form class="col-md-12 layui-form"  enctype="multipart/form-data" method="post" id="form" class="center-block " style=" margin: 0 auto;">
            <div class="col-md-4">
                <div class="form-group">
                    <label for="name" class="col-sm-2 control-label">邮件标题</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control" name="title" id="title" placeholder="邮件标题" value="">
                    </div>
                </div>
                <div class="form-group">
                    <label for="name" class="col-sm-2 control-label">接受邮箱</label>
                    <div class="col-sm-10">
                        <input type="text" class="form-control" name="email" id="email" placeholder="接受邮箱" value="">
                    </div>
                </div>
                {{ csrf_field() }}
                <!-- 加载编辑器的容器 -->
...
<script type="text/javascript">
    ...
    layui.use(['form','layedit'], function(){
        ...
        form.on('submit(formDemo)', function(data){
            ...
            $.post("{{url('admin/email')}}",$('#form').serialize(),
                function(data){
                    console.log(data)
                    if(data.status == 1){
                        layer.msg(data.info);
